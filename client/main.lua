        ...
        while true do
            ...
            if LocalPlayer.state.isLoggedIn then
                ...
                if entering ~= 0 and not isBlacklistedVehicle(entering) then
                    sleep = 2000
                    local plate = KGCore.Functions.GetPlate(entering)
                    ...
                    if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                        if IsEntityDead(driver) then
                            if not isTakingKeys then
                                ...
                                TriggerServerEvent('kg-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                                KGCore.Functions.Progressbar('steal_keys', Lang:t('progress.takekeys'), 2500, false, false, {
                                    disableMovement = false,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true
                                }, {}, {}, {}, function() -- Done
                                    TriggerServerEvent('kg-vehiclekeys:server:AcquireVehicleKeys', plate)
                                    isTakingKeys = false
                                end, function()
                                    isTakingKeys = false
                                end)
                        ...
                        elseif Config.LockNPCDrivingCars then
                            TriggerServerEvent('kg-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                        else
                            TriggerServerEvent('kg-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            TriggerServerEvent('kg-vehiclekeys:server:AcquireVehicleKeys', plate)
                    ...
                    elseif driver == 0 and entering ~= lastPickedVehicle and not HasKeys(plate) and not isTakingKeys then
                        KGCore.Functions.TriggerCallback('kg-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() and KGCore.Functions.GetPlayerData() ~= {} then
        GetKeys()
    end
end)
...
RegisterNetEvent('kg-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local vehicle = GetVehiclePedIsIn(ped)
...
RegisterNetEvent('kg-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)

RegisterNetEvent('kg-vehiclekeys:client:ToggleEngine', function()
    local EngineOn = GetIsVehicleEngineRunning(GetVehiclePedIsIn(PlayerPedId()))
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if HasKeys(KGCore.Functions.GetPlate(vehicle)) then
        if EngineOn then
            SetVehicleEngineOn(vehicle, false, false, true)
        else
...
RegisterNetEvent('kg-vehiclekeys:client:GiveKeys', function(id)
    local targetVehicle = GetVehicle()
    if targetVehicle then
        local targetPlate = KGCore.Functions.GetPlate(targetVehicle)
            ...
            if id and type(id) == 'number' then -- Give keys to specific ID
                ...
                if IsPedSittingInVehicle(PlayerPedId(), targetVehicle) then -- Give keys to everyone in vehicle
                    ...
                    for p = 1, #otherOccupants do
                        TriggerServerEvent('kg-vehiclekeys:server:GiveVehicleKeys', GetPlayerServerId(NetworkGetPlayerIndexFromPed(otherOccupants[p])), targetPlate)
                    end
                else -- Give keys to closest player
                    GiveKeys(GetPlayerServerId(KGCore.Functions.GetClosestPlayer()), targetPlate)
                end
            end
        else
            KGCore.Functions.Notify(Lang:t('notify.ydhk'), 'error')
        end
    end
end)

RegisterNetEvent('KGCore:Client:EnteringVehicle', function()
    robKeyLoop()
end)

RegisterNetEvent('kg-weapons:client:DrawWeapon', function()
    Wait(2000)
    robKeyLoop()
end)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    local vehicle = KGCore.Functions.GetClosestVehicle()

    if vehicle == nil or vehicle == 0 then return end
    if HasKeys(KGCore.Functions.GetPlate(vehicle)) then return end
    if #(pos - GetEntityCoords(vehicle)) > 2.5 then return end
    if GetVehicleDoorLockStatus(vehicle) <= 0 then return end

    local difficulty = isAdvanced and 'easy' or 'medium' -- Easy for advanced lockpick, medium by default
    local success = exports['kg-minigames']:Skillbar(difficulty)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('kg-vehiclekeys:server:AcquireVehicleKeys', KGCore.Functions.GetPlate(vehicle))
        else
            KGCore.Functions.Notify(Lang:t('notify.vlockpick'), 'success')
            TriggerServerEvent('kg-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(vehicle), 1)
        end
    else
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
    ...
    if isAdvanced then
        if chance <= Config.RemoveLockpickAdvanced then
            TriggerServerEvent('kg-vehiclekeys:server:breakLockpick', 'advancedlockpick')
        end
    else
        if chance <= Config.RemoveLockpickNormal then
            TriggerServerEvent('kg-vehiclekeys:server:breakLockpick', 'lockpick')
        end
    end
end)
...
function ToggleEngine(veh)
    if veh then
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(KGCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                if EngineOn then
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(KGCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    TriggerServerEvent('kg-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 2)
                    KGCore.Functions.Notify(Lang:t('notify.vlock'), 'primary')
                else
                    TriggerServerEvent('kg-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
                    KGCore.Functions.Notify(Lang:t('notify.vunlock'), 'success')
                end
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        TriggerServerEvent('kg-vehiclekeys:server:GiveVehicleKeys', id, plate)
    else
        KGCore.Functions.Notify(Lang:t('notify.nonear'), 'error')
    end
end

function GetKeys()
    KGCore.Functions.TriggerCallback('kg-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
...
function GetVehicle()
    ...
    while vehicle == 0 do
        vehicle = KGCore.Functions.GetClosestVehicle()
...
function AreKeysJobShared(veh)
    ...
    local vehPlate = KGCore.Functions.GetPlate(veh)
    local jobName = KGCore.Functions.GetPlayerData().job.name
    local onDuty = KGCore.Functions.GetPlayerData().job.onduty
            ...
            for _, vehicle in pairs(v.vehicles) do
                if string.upper(vehicle) == string.upper(vehName) then
                    if not HasKeys(vehPlate) then
                        TriggerServerEvent('kg-vehiclekeys:server:AcquireVehicleKeys', vehPlate)
                    end
                    return true
                end
            end
...
function Hotwire(vehicle, plate)
    ...
    KGCore.Functions.Progressbar('hotwire_vehicle', Lang:t('progress.hskeys'), hotwireTime, false, true, {
        ...
        if (math.random() <= Config.HotwireChance) then
            ...
            KGCore.Functions.Notify(Lang:t('notify.fvlockpick'), 'error')
        end
...
function CarjackVehicle(target)
    ...
    KGCore.Functions.Progressbar('rob_keys', Lang:t('progress.acjack'), Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
        ...
        if hasWeapon and isCarjacking then
            ...
            if math.random() <= carjackChance then
                ...
                KGCore.Functions.Notify(Lang:t('notify.cjackfail'), 'error')
                FreezeEntityPosition(vehicle, false)
                SetVehicleUndriveable(vehicle, false)
                MakePedFlee(target)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
...
function DrawText3D(x, y, z, text)
    ...
    if GetConvar('kg_locale', 'en') == 'en' then
        SetTextFont(4)
    else
        SetTextFont(1)
    end
